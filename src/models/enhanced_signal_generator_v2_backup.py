...
class EnhancedSignalGeneratorV2:
    ...
    def load_model(self, model_path: str):
        ...
        try:
            ...
            try:
                ...
                try:
                    # Method 2: Loading with weights_only=False for newer PyTorch
                    model_data = torch.load(model_path, map_location='cpu', weights_only=False)
                except Exception as e2:
                    logger.error(f"Failed to load model with both methods: {e1}, {e2}")
                    return None
            ...
            if isinstance(model_data, dict):
                if 'model' in model_data:
                    ...
                    logger.warning("Model state_dict found but no model architecture")
                    return None
                else:
    ...
    def generate_v5_prediction(self, symbol: str, features: Dict) -> Tuple[float, float]:
        ...
        try:
            ...
            # Make prediction
            with torch.no_grad():
                if hasattr(self.v5_model, 'predict'):
                    prediction = self.v5_model.predict(feature_vector)
                else:
                    # Try forward pass
    ...
    def generate_bulk_signals(self, symbols: List[str], max_workers: int = 8) -> List[Dict]:
        """Generate signals for multiple symbols efficiently"""
