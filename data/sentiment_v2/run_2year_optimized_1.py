#!/usr/bin/env python3
"""
Aggressive 2-Year Optimized Parallel Sentiment Analysis Runner - Batch 1 (Stocks 1-28)
=====================================================================================

This script runs sentiment analysis for the first batch of stocks (1-28) for 2 years (2024-2025)
...
class Aggressive2YearOptimizedRunner1:
    """Aggressive 2-year optimized parallel sentiment analysis runner for batch 1 (stocks 1-28)"""
    
    def __init__(self):
        """Initialize the aggressive 2-year optimized parallel sentiment runner"""
        self.setup_logging()
        self.logger.info("Initializing Aggressive 2-Year Optimized Parallel Sentiment Runner - Batch 1")
        ...
        # Load stocks 1-28 (first batch)
        self.stock_symbols = self.load_batch_1_stocks()
        ...
        self.output_dir = Path("2year_parallel_sentiment_batch_1")
    ...
    def setup_logging(self):
        ...
        log_file = f"2year_parallel_batch_1_sentiment_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        self.logger = logging.getLogger("aggressive_2year_parallel_sentiment_runner_1")
    
    def load_batch_1_stocks(self) -> List[str]:
        """Load stocks 1-28 for batch 1"""
        return [
            ...
            "ONGC.NSE", "JSWSTEEL.NSE", "TATAMOTORS.NSE"
        ]
    ...
    def calculate_total_workload(self) -> Dict[str, Any]:
        """Calculate the total workload for 2-year batch 1"""
    ...
    def run_complete_analysis(self):
        """Run the complete 2-year sentiment analysis for batch 1"""
        workload = self.calculate_total_workload()
        
        self.logger.info("=" * 80)
        self.logger.info("STARTING AGGRESSIVE 2-YEAR PARALLEL BATCH 1 SENTIMENT ANALYSIS")
        self.logger.info("=" * 80)
        ...
        try:
            # Step 1: Run comprehensive backfill for batch 1 stocks and dates
            self.logger.info("Step 1: Running comprehensive backfill for batch 1 (2 years)...")
            ...
            self.logger.info("Step 2: Generating comprehensive dataset for batch 1 (2 years)...")
            ...
            daily_rollup = self.generator.rollup_aggregate(
                ...
                "D", 
                "2year_daily_sentiment_rollup_batch_1.csv"
            )
            ...
            weekly_rollup = self.generator.rollup_aggregate(
                "comprehensive_sentiment_dataset.csv", 
                "W", 
                "2year_weekly_sentiment_rollup_batch_1.csv"
            )
            ...
            monthly_rollup = self.generator.rollup_aggregate(
                "comprehensive_sentiment_dataset.csv", 
                "M", 
                "2year_monthly_sentiment_rollup_batch_1.csv"
            )
            ...
            # Save final report
            report_file = self.output_dir / "2year_batch_1_final_report.json"
            with open(report_file, 'w') as f:
                json.dump(final_report, f, indent=2)
            
            self.logger.info("=" * 80)
            self.logger.info("AGGRESSIVE 2-YEAR BATCH 1 ANALYSIS COMPLETED SUCCESSFULLY")
            self.logger.info("=" * 80)
            ...
            self.logger.error(f"Error in aggressive 2-year batch 1 analysis: {e}")
            raise
    ...
    def print_summary(self, report: Dict[str, Any]):
        ...
        self.logger.info("")
        self.logger.info("📊 AGGRESSIVE 2-YEAR BATCH 1 ANALYSIS SUMMARY")
        self.logger.info("=" * 60)
...
def main():
    ...
    try:
        runner = Aggressive2YearOptimizedRunner1()
        report = runner.run_complete_analysis()
        
        print("\n" + "="*80)
        print("✅ AGGRESSIVE 2-YEAR BATCH 1 ANALYSIS COMPLETED SUCCESSFULLY!")
        print("="*80)
        ...
        print(f"❌ Error in aggressive 2-year batch 1 analysis: {e}")
        sys.exit(1)
