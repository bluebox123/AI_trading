#!/usr/bin/env python3
"""
Complete 18-Month Sentiment Analysis Runner - BATCH 2
====================================================

Parallel processing script for BATCH 2 (40 stocks):
- Stocks 41-80: EICHERMOT.NSE to LTTS.NSE
- 18 months: January 2nd, 2024 to July 7th, 2025
- Intraday sentiment data (every single day)
- All advanced features enabled
- Independent parallel processing

This is BATCH 2 of 3 parallel scripts for faster processing.
"""
...
class CompleteSentimentRunnerBatch2:
    """Complete 18-month sentiment analysis runner - BATCH 2"""
    
    def __init__(self):
        """Initialize the complete sentiment runner for batch 2"""
        self.batch_id = 2
        self.setup_logging()
        self.logger.info("Initializing Complete Sentiment Runner - BATCH 2")
        ...
        # Load BATCH 2 stock symbols (stocks 41-80)
        self.stock_symbols = self.load_batch2_stock_symbols()
        ...
        self.output_dir = Path("complete_18month_sentiment_dataset_batch2")
        self.output_dir.mkdir(exist_ok=True)
        
        self.logger.info(f"Loaded {len(self.stock_symbols)} stock symbols for BATCH 2")
        self.logger.info(f"Date range: {self.start_date} to {self.end_date}")
        self.logger.info(f"Output directory: {self.output_dir}")
    
    def setup_logging(self):
        """Setup comprehensive logging for batch 2"""
        log_file = f"complete_18month_sentiment_batch2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        logging.basicConfig(
            level=logging.INFO,
        ...
        self.logger = logging.getLogger("complete_sentiment_runner_batch2")
    
    def load_batch2_stock_symbols(self) -> List[str]:
        """Load BATCH 2 stock symbols (stocks 41-80)"""
        # BATCH 2: Stocks 41-80
        batch2_stocks = [
        ...
        self.logger.info(f"Loaded {len(batch2_stocks)} stock symbols for BATCH 2")
        return batch2_stocks
    
    def calculate_total_workload(self) -> Dict[str, Any]:
        """Calculate the total workload for batch 2"""
    ...
    def run_complete_analysis(self):
        """Run the complete 18-month sentiment analysis for batch 2"""
        workload = self.calculate_total_workload()
        
        self.logger.info("=" * 80)
        self.logger.info("STARTING COMPLETE 18-MONTH SENTIMENT ANALYSIS - BATCH 2")
        self.logger.info("=" * 80)
        ...
        try:
            # Step 1: Run comprehensive backfill for batch 2 stocks
            self.logger.info("Step 1: Running comprehensive backfill for BATCH 2...")
            ...
            self.logger.info("Step 2: Generating comprehensive dataset for BATCH 2...")
            ...
            daily_rollup = self.generator.rollup_aggregate(
                ...
                "D", 
                "daily_sentiment_rollup_batch2.csv"
            )
            ...
            weekly_rollup = self.generator.rollup_aggregate(
                "comprehensive_sentiment_dataset.csv", 
                "W", 
                "weekly_sentiment_rollup_batch2.csv"
            )
            ...
            monthly_rollup = self.generator.rollup_aggregate(
                "comprehensive_sentiment_dataset.csv", 
                "M", 
                "monthly_sentiment_rollup_batch2.csv"
            )
            ...
            final_report = {
                ...
                "missing_data_summary": {
                    ...
                    "missing_data_log": "missing_data_log_batch2.json"
                }
            }
            
            # Save final report
            with open(self.output_dir / "complete_18month_analysis_report_batch2.json", 'w') as f:
                json.dump(final_report, f, indent=2)
            
            self.logger.info("=" * 80)
            self.logger.info("COMPLETE 18-MONTH SENTIMENT ANALYSIS FINISHED - BATCH 2")
            self.logger.info("=" * 80)
            ...
            self.logger.error(f"Error during BATCH 2 analysis: {e}")
            raise
    
    def print_summary(self, report: Dict[str, Any]):
        """Print a summary of the batch 2 analysis results"""
        print("\n" + "=" * 80)
        print("COMPLETE 18-MONTH SENTIMENT ANALYSIS SUMMARY - BATCH 2")
        print("=" * 80)
        
        workload = report["analysis_summary"]["workload"]
        duration = report["analysis_summary"]["duration_hours"]
        
        print(f"📊 Analysis Coverage (Batch 2):")
        print(f"   • Date Range: {workload['start_date']} to {workload['end_date']}")
        print(f"   • Total Days: {workload['total_days']:,}")
        print(f"   • Total Stocks: {workload['total_stocks']} (Batch 2)")
...
def main():
    """Main function to run the complete 18-month sentiment analysis - BATCH 2"""
    print("🚀 Starting Complete 18-Month Sentiment Analysis - BATCH 2")
    print("This will process 40 stocks (EICHERMOT.NSE to NESTLEIND.NSE) for 18 months")
    print("Expected duration: ~10 hours")
    print("Output: Comprehensive sentiment dataset for BATCH 2")
    print("\nPress Enter to continue or Ctrl+C to cancel...")
    
    try:
        input()
    except KeyboardInterrupt:
        print("\n❌ BATCH 2 analysis cancelled by user")
        return
    
    # Run the complete analysis for batch 2
    runner = CompleteSentimentRunnerBatch2()
    report = runner.run_complete_analysis()
    
    # Print summary
    runner.print_summary(report)
    
    print("\n✅ Complete 18-month sentiment analysis BATCH 2 finished successfully!")
